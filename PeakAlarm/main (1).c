...
void steileStellenNachObenMelden(const float *daten)
{
...
void steileStellenNachUntenMelden(const float *daten)
{
    for(int i=1;i<ANZAHL;i++)
    {
        if(daten[i]<daten[i-1]-daten[i-1]*25.0/100.0)
        {
            printf("Alarm! An Index %d fällt der Wert um mehr als 25%%\n",i);
        }
    }
}
